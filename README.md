...
- **Claude Opus** (`claude-opus-5-5`, `claude-opus-5`, `claude-opus-4-8`, `claude-opus-4-7`, `claude-opus-4-6`, `claude-opus-4-5-20251101`)
...
Claude Opus 5.5 (`claude-opus-5-5`) is the recommended starting point for most workloads: a 1M-token context window and 128K max output, **always-on adaptive thinking** (effort defaults to `medium`), and a lower price than Opus 5 ($4/$20 per MTok, cache reads $0.20 per MTok). Like Fable 5.1, its thinking blocks are bound to the conversation prefix, which the pipe handles — see [What's new in Claude Opus 5.5](docs/claude_opus_5-5.md).

Claude Opus 5 (`claude-opus-5`) remains available: adaptive thinking on by default (can be disabled at effort `high` or below), the full `low`-`max` effort range, and $5/$25 per MTok. See [What's new in Claude Opus 5](docs/claude_opus_5.md).
